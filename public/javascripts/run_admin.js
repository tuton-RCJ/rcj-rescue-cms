var socket;
var app = angular.module("RunAdmin", ['ngAnimate', 'ui.bootstrap', 'ui.bootstrap.datetimepicker']);
app.controller('RunAdminController', ['$scope', '$http', '$log', '$location', function ($scope, $http, $log, $location) {
  ...
  $http.get("/api/competitions/" + competitionId +
            "/"+league+"/teams").then(function (response) {
    $scope.teams = response.data
  })
  $http.get("/api/competitions/" + competitionId +
            "/"+league+"/rounds").then(function (response) {
    $scope.rounds = response.data
  })
  $http.get("/api/competitions/" + competitionId +
            "/"+league+"/fields").then(function (response) {
    $scope.fields = response.data
  })
  $http.get("/api/competitions/" + competitionId +
            "/"+league+"/maps").then(function (response) {
  ...
  $scope.addRun = function () {
    ...
    $http.post("/api/runs/", run).then(function (response) {
  ...
  $scope.removeRun = function (run) {
    swal({
      ...
      $http.delete("/api/runs" + run._id).then(function (response) {
        console.log(response)
        updateRunList()
      }, function (error) {
        console.log(error)
  ...
  function updateRunList() {
    $http.get("/api/competitions/" + competitionId +
              "/"+league+"/runs?populate=true").then(function (response) {
