...
app.controller("CabinetFileController", ['$scope', '$http', '$translate', 'Upload', function ($scope, $http, $translate, Upload) {
    ...
    $scope.download = function(file){
        ...
        alink.href = `/api/cabinet/${competitionId}/file/${teamId}/${token}/${encodeURIComponent(file.folder)}/${file.encoded}`;
        alink.click();
        return false;
    }
    ...
    $scope.delete = function(file){
        Swal.fire({
            ...
            if (result.value) {
                $http.delete(`/api/cabinet/${competitionId}/file/${encodeURIComponent(file.folder)}/${file.encoded}`).then(function (response) {
                    Swal.fire(
                        trans['delComplete'],
